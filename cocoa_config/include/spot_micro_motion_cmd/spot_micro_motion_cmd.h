...
class SpotMicroMotionCmd
{
 ...
 public:
  ...
  // Publishes the joint states of the robot after publishServoProportionalCommand();
  // based on the trajectories from jointTrajCallback
 ...
 private:
  ...
  // NOTE: FOR CHAMP INTEGRATION, CMD SHOULD ALWAYS BE IN WALK_CMD_ = TRUE
  ...
  ros::Subscriber joint_traj_sub_;             // ros subscriber handle for joint trajectories from CHAMP
  ...
  ros::Publisher joint_state_pub_;               // joint state publisher from CHAMP
  ...
  // Callback method for Joint trajectory info commands
