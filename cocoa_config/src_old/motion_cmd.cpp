...
void SpotMicroMotionCmd::jointTrajCallback(const trajectory_msgs::JointTrajectory &msg){
  trajectory_msgs::JointTrajectory traj = msg;

  // NOTE: cmd_ needs to be set to walk_ in order to always use champ's joint trajectories instead of spotmicro's control scheme
  cmd_.walk_cmd_ = true;

  for (int i = 0; i < traj.joint_names.size(); i++){
    //ROS_INFO("JOINT TRAJ HEARD");
    // Map the joint trajectories to the servo_cmds_rad_

    if ( "coxaF_FL" == traj.joint_names[i])
      servo_cmds_rad_["LF_1"] = traj.points[0].positions[i];
    if ( "femurF_FL" == traj.joint_names[i])
      servo_cmds_rad_["LF_2"] = traj.points[0].positions[i];
    if ( "tibiaF_FL" == traj.joint_names[i])
      servo_cmds_rad_["LF_3"] = traj.points[0].positions[i];
    if ( "coxaF_FR" == traj.joint_names[i])
      servo_cmds_rad_["RF_1"] = traj.points[0].positions[i];
    if ( "femurF_FR" == traj.joint_names[i])
      servo_cmds_rad_["RF_2"] = traj.points[0].positions[i];
    if ( "tibiaF_FR" == traj.joint_names[i])
      servo_cmds_rad_["RF_3"] = -traj.points[0].positions[i]; // NOTE: joint off
    if ( "coxaF_BL" == traj.joint_names[i])
      servo_cmds_rad_["LB_1"] = traj.points[0].positions[i];
    if ( "femurF_BL" == traj.joint_names[i])
      servo_cmds_rad_["LB_2"] = traj.points[0].positions[i];
    if ( "tibiaF_BL" == traj.joint_names[i])
      servo_cmds_rad_["LB_3"] = traj.points[0].positions[i];
    if ( "coxaF_BR" == traj.joint_names[i])
      servo_cmds_rad_["RB_1"] = traj.points[0].positions[i];
    if ( "femurF_BR" == traj.joint_names[i])
      servo_cmds_rad_["RB_2"] = traj.points[0].positions[i];
    if ( "tibiaF_BR" == traj.joint_names[i])
      servo_cmds_rad_["RB_3"] = -traj.points[0].positions[i];  // NOTE: joint off

    joint_states_.name[i] = traj.joint_names[i];
    joint_states_.position[i] = traj.points[0].positions[i];
  }
...
SpotMicroMotionCmd::SpotMicroMotionCmd(ros::NodeHandle &nh, ros::NodeHandle &pnh) {
  ...
  // Set joint states names and message size;
  joint_states_.name.resize(12);      // 12 links total in all 4 legs
  joint_states_.position.resize(12);  // 12 links total in all 4 legs
  ...
  joint_traj_sub_ = nh.subscribe("/joint_group_position_controller/command", 1, &SpotMicroMotionCmd::jointTrajCallback, this);;
...
void SpotMicroMotionCmd::publishJointState(){
  // [coxaF_FL, femurF_FL, tibiaF_FL, coxaF_FR, femurF_FR, tibiaF_FR, coxaF_BL, femurF_BL, tibiaF_BL, coxaF_BR, femurF_BR, tibiaF_BR]
  /*
  std::string robot_id = "quadruped";
  char *link_name[12] = {"coxaF_FL", "femurF_FL", "tibiaF_FL", "coxaF_FR", "femurF_FR", "tibiaF_FR", "coxaF_BL", "femurF_BL", "tibiaF_BL", "coxaF_BR", "femurF_BR", "tibiaF_BR"};
  float pos[12];

  for (int i = 0; i < joint_states.position_length; i++){
    pos[i] = 1;
  }

  joint_states.name = link_name;
...
void SpotMicroMotionCmd::runOnce() {
  ...
  //resetEventCommands(); //NOTE: not resetting in order to speed up and avoid any conflicts between controllers
