# ALTERNATIVE GAIT GENERATION TO FIX THE SINGULARITY BEHAVIORS


# TODO: EDIT THE QUADRUPED_DESCRIPTION.H so the numbers MATCH UP WITH YOUR MEASUREMENTS (FROM PICTURE)
...
#### 1.1.1. Run the base driver:
    ...
    roslaunch cocoa2_config bringup.launch rviz:=true
...
#### 1.2.1. Run the Gazebo environment:
    ...
    roslaunch cocoa2_config gazebo.launch 

#### 1.2.2. Run gmapping package and move_base:

    roslaunch cocoa2_config slam.launch rviz:=true
...
#### 1.3.2. Run amcl and move_base:
    ...
    roslaunch cocoa2_config navigate.launch rviz:=true
...
Spawning a robot:
    ...
    roslaunch cocoa2_config spawn_robot.launch robot_name:=<unique_robot_name> world_init_x:=<x_position> world_init_y:=<y_position>
